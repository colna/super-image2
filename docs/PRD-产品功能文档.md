...
| 功能          | 说明                                                                              |
| ------------- | --------------------------------------------------------------------------------- |
| Provider 选择 | 下拉选择服务商，MVP 仅支持 OpenAI（首要支持 gpt-image-2），架构预留扩展口         |
| API Key 输入  | 密码框，本地加密存储（localStorage + AES），不上传服务端                          |
| Base URL 输入 | 可选，默认 `https://api.openai.com/v1`，支持自定义代理地址                        |
| 模型选择      | 根据 Provider 展示可用模型列表（如 `gpt-image-2`、`dall-e-3`）                    |
...
- [x] OpenAI Provider 配置与连通性测试（首要支持 gpt-image-2）
